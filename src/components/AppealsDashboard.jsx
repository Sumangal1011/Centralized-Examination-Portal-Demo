...
export default function AppealsDashboard({ token }) {
  ...
  const fetchAppeals = async () => {
    try {
      ...
      setAppeals(data);
      if (selectedAppeal) {
        const updated = data.find((a) => a._id === selectedAppeal._id);
  ...
  return (
    <div id="appeals-dashboard-root" className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 border border-slate-850 p-4 rounded-xl relative overflow-hidden">
      ...
      {msg && (
        <div className="p-3 bg-teal-955/40 border border-teal-500/30 text-xs text-white rounded-lg flex items-center gap-2">
            ...
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              <div className="relative flex-1 sm:flex-initial">
                ...
                <input
                  ...
                  className="w-full sm:w-48 bg-slate-950 border border-slate-850 rounded-lg py-1.5 pl-8 pr-4 text-xs font-mono text-slate-200 placeholder:text-slate-650 focus:outline-none focus:border-slate-755"
                />
              </div>
                ...
                {filtered.map((appeal) => (
                  <tr 
                    ...
                    className={`hover:bg-slate-950/20 transition-all ${
                      selectedAppeal?._id === appeal._id ? "bg-slate-955/40 border-l-2 border-teal-500" : ""
                    }`}
                  >
                    ...
                    <td className="p-3 text-slate-505">{appeal.violationType}</td>
                    ...
                    <td className="p-3 text-right">
                      <button
                        ...
                        className="px-2.5 py-1 bg-slate-800 hover:bg-slate-705 text-[10px] font-bold text-slate-200 rounded border border-slate-700 transition-colors uppercase cursor-pointer"
                      >
                        Investigate
                      </button>
                    </td>
          ...
          {selectedAppeal ? (
            <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl space-y-4 shadow-xl">
              ...
              <div className="bg-slate-950 p-3.5 rounded-lg border border-slate-850/60 space-y-2">
                <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500 block">Examinee Narrative</span>
              ...
              {selectedAppeal.status === "Pending" ? (
                <div className="space-y-3.5 pt-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <button
                      ...
                      disabled={resolving}
                      className="py-2 bg-red-500/10 hover:bg-red-500/20 text-red-00 hover:text-red-300 border border-red-500/30 text-xs font-mono font-bold uppercase rounded-lg transition-colors cursor-pointer flex items-center justify-center gap-1"
                    >
                ...
                <div className="bg-slate-955 p-3.5 rounded-lg border border-slate-805 space-y-1.5 font-mono text-xs text-slate-300">
