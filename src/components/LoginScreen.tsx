...
interface LoginScreenProps {
  onLoginSuccess: (token: string, user: { username: string; role: "student" | "examiner" | "admin"; name: string }) => void;
}

export default function LoginScreen({ onLoginSuccess }: LoginScreenProps) {
  const [role, setRole] = useState<"student" | "examiner" | "admin">("examiner");
  ...
  const handleRoleChange = (selectedRole: "student" | "examiner" | "admin") => {
    ...
    <div id="login-screen-root" className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 selection:bg-teal-500 selection:text-slate-950 font-sans">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300">
        ...
        <div className="bg-slate-950 p-6 border-b border-slate-800 text-center flex flex-col items-center relative">
          ...
          <div className="w-12 h-12 rounded-xl bg-gradient-to-tr from-teal-500 to-cyan-500 flex items-center justify-center text-slate-950 mb-3 shadow-[0_0_20px_rgba(20,184,166,0.3)]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            <span className="text-xs font-mono text-slate-400 uppercase tracking-wider">Select Credentials Scope</span>
            <div className="grid grid-cols-3 gap-1 p-1 bg-slate-950/80 rounded-lg border border-slate-800/80">
              {(["student", "examiner", "admin"] as const).map((r) => (
                <button
                  key={r}
                  type="button"
          ...
          {/* Alert Message */}
          {error && (
            <div className="p-3 bg-red-950/50 border border-red-500/30 rounded-lg flex items-start gap-2.5 text-xs text-red-300">
          ...
          <div className="space-y-3">
            {isRegister && (
              <div className="space-y-1">
                <label className="text-xs font-mono text-slate-400 uppercase">Aesthetic Title / Full Name</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg py-2 pl-9 pr-4 text-sm text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-teal-500/70 transition-colors"
                  />
                </div>
              </div>
            )}
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[10px] text-teal-400 font-mono hover:underline cursor-pointer">SSO Active</span>
              </div>
              <div className="relative">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-slate-950 font-semibold rounded-lg py-2.5 text-sm transition-all focus:outline-none shadow-md shadow-teal-500/10 cursor-pointer flex justify-center items-center gap-1.5"
          >
        ...
        <div className="bg-slate-950/80 px-6 py-4 border-t border-slate-800/60 flex flex-col gap-1.5">
          <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest text-center">Demo Fast Credentials</div>
          <div className="grid grid-cols-2 gap-2 text-[11px] font-mono text-slate-400">
            <div className="bg-slate-900 border border-slate-800 p-1.5 rounded text-center">
              <span className="text-teal-400 font-bold block">STUDENT MODE</span>
              <span className="text-[10px] text-slate-500">student / student123</span>
            </div>
            <div className="bg-slate-900 border border-slate-800 p-1.5 rounded text-center">
              <span className="text-cyan-400 font-bold block">EXAMINER MODE</span>
