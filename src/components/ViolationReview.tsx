...
export default function ViolationReview({ token, selectedStudentName, onAdjudicationSuccess }: ViolationReviewProps) {
  const [incident, setIncident] = useState<ViolationIncident | null>(null);
  const [isPlaying, setIsPlaying] = useState(false);
  const [timelineSec, setTimelineSec] = useState(45);
  const [verdict, setVerdict] = useState<"Dismissed" | "Warned" | "Nullified">("Warned");
  ...
  // Load first matching incident for the specified student (defaults to Alex Johnson)
  const fetchIncident = async () => {
    try {
      const res = await fetch("/api/violation-incidents");
      const data = await res.json();
      const match = data.find((i: any) => i.studentName === selectedStudentName);
      if (match) {
        setIncident(match);
  ...
  const handleSubmittingAdjudication = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!incident) return;
    setSubmitting(true);
    setFeedbackMsg(null);
  ...
  if (!incident) {
    return (
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center text-slate-500 font-mono">
  ...
  return (
    <div id="violation-review-root" className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-7 space-y-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden relative shadow-lg">
          ...
          <div className="aspect-video bg-slate-950 relative flex items-center justify-center border-b border-slate-800">
            {/* Main Pixel Frame Representing Alexandr */}
            <div className="text-center space-y-3 relative z-10 p-4">
              <div className="w-32 h-32 mx-auto rounded-full bg-slate-900 border border-slate-800 p-1 flex items-center justify-center relative">
                ...
                <span className="absolute -top-3.5 left-1/2 transform -translate-x-1/2 px-1.5 py-0.5 bg-red-600 border border-red-500 text-[8px] font-mono font-bold text-white uppercase rounded whitespace-nowrap">
            ...
            <div className="absolute bottom-4 right-4 font-mono text-[10px] text-red-400 space-y-1 bg-slate-950/80 p-1.5 rounded border border-slate-800 animate-pulse">
              <div>MUTED: FALSE</div>
          ...
          <div className="p-4 bg-slate-950/60 flex flex-col gap-3">
            <div className="flex items-center justify-between gap-4">
              <button
                onClick={() => setIsPlaying(!isPlaying)}
                className="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-xs font-mono font-bold uppercase rounded text-white flex items-center gap-1.5 border border-slate-700/60 transition-colors cursor-pointer"
              >
              ...
              <div className="flex-1 flex items-center gap-3 font-mono text-xs">
                <span className="text-slate-500 font-medium">{formatTime(timelineSec)}</span>
                <input
                  type="range"
                  min="0"
                  max="120"
                ...
                />
                <span className="text-slate-500 font-medium">02:00</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <div key={index} className="py-2.5 flex justify-between items-start gap-4 text-xs font-mono">
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-slate-400 text-[11px] block mt-0.5">{log.description}</span>
                  </div>
                </div>
      ...
      <div className="lg:col-span-5">
        ...
        <form onSubmit={handleSubmittingAdjudication} className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-5 h-full flex flex-col justify-between">
          <div className="space-y-4">
            ...
            {feedbackMsg && (
              <div className="p-3 bg-teal-950/40 border border-teal-500/30 rounded-lg text-xs text-teal-300 flex items-start gap-2">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 gap-2">
                ...
                <label className={`p-3 rounded-xl border transition-all cursor-pointer flex justify-between items-center ${
                  verdict === "Dismissed" 
                    ? "bg-emerald-950/30 border-emerald-500 text-teal-200" 
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-400 leading-normal">Exonerate candidate, clear proctor counters.</span>
                  </div>
                  <input
                    type="radio"
                    name="verdict"
                ...
                <label className={`p-3 rounded-xl border transition-all cursor-pointer flex justify-between items-center ${
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-400 leading-normal">Subtract integrity scoring points, notify applicant.</span>
                  </div>
                  <input
                    type="radio"
                    name="verdict"
                ...
                <label className={`p-3 rounded-xl border transition-all cursor-pointer flex justify-between items-center ${
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-slate-400 leading-normal">Force-submit with 0% absolute grade, notify Dean structures.</span>
                  </div>
                  <input
                    type="radio"
                    name="verdict"
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full min-h-[100px] bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-slate-700 leading-normal"
              />
            </div>

            {/* Verification Toggles */}
            <div className="space-y-2 bg-slate-950/40 p-3 rounded-lg border border-slate-850">
              <label className="text-[10px] font-mono uppercase tracking-wider text-slate-500 font-bold block">Integrity Notifications</label>
              ...
              <label className="flex items-center gap-2.5 text-xs text-slate-300 cursor-pointer select-none">
                ...
                />
                <span>Notify student immediately via secure post portal</span>
              </label>
              ...
              <label className="flex items-center gap-2.5 text-xs text-slate-300 cursor-pointer select-none">
                <input
                  ...
                  className="accent-teal-500 h-3.5 w-3.5 rounded cursor-pointer"
                />
                <span>Escalate case documentation to Department Dean</span>
              </label>
            </div>
          </div>
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-2.5 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 text-slate-950 font-bold font-mono text-xs uppercase tracking-wider rounded-lg shadow-md hover:shadow-teal-500/10 cursor-pointer flex justify-center items-center gap-1.5 transition-all mt-4"
          >
